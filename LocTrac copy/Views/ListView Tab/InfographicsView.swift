...
struct InfographicsView: View {
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                ...
                LazyVStack(spacing: 24, pinnedViews: []) {
                    ...
                    if let derived = derivedByYear[selectedYear] {
                        ...
                        .id(selectedYear) // Force view identity change for instant updates
                    } else {
                        // Loading state
                        VStack(spacing: 16) {
                            ProgressView()
            ...
            .animation(nil, value: selectedYear) // Disable implicit animations on year change
        ...
        .task(id: selectedYear) {
            ...
            if derivedByYear[selectedYear] != nil {
                print("✅ Derived data already computed for \(selectedYear) - using cache")
                
                // Precompute adjacent years in background for instant switching
            ...
            print("⏳ No cache for \(selectedYear) - computing...")
            
            // Compute derived data in background
            await computeDerivedData(for: selectedYear)
            
            // After computing current year, precompute adjacent years
            Task.detached(priority: .background) {
                await precomputeAdjacentYears(current: selectedYear)
            }
        }
    ...
    // MARK: - Precompute Adjacent Years
    
    /// Precompute adjacent years in the background for instant switching
    private func precomputeAdjacentYears(current: String) async {
        guard let currentIndex = availableYears.firstIndex(of: current) else { return }
        
        // Get adjacent years
        var yearsToPrecompute: [String] = []
        
        // Previous year
        if currentIndex > 0 {
            yearsToPrecompute.append(availableYears[currentIndex - 1])
        }
        ...
        // Compute them in background
        for year in yearsToPrecompute {
            // Check if already computed
            let alreadyComputed = await MainActor.run {
                derivedByYear[year] != nil
            }
            
            if !alreadyComputed {
                print("🔮 Precomputing \(year) in background...")
