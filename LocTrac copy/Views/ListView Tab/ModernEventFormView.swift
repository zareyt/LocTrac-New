...
struct ModernEventFormView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                if viewModel.location?.name == "Other" {
                    coordinatesSection
                }
    ...
    private var sortedLocations: [Location] {
        // Check if there's a default location set
        ...
        // If no default, just return locations as-is
    ...
    private var eventTypeSection: some View {
        Section {
            ...
            .pickerStyle(.navigationLink) // Make it obvious - shows as a tappable row
        } header: {
        ...
        Section {
            if store.activities.isEmpty {
                HStack {
                    ...
                    Spacer()
                    Button("Manage") {
                        // This could open activities management
                    }
                    .font(.subheadline)
                }
            } else {
    ...
    private func createNewEvents() async {
        ...
        // Get country once (all events have same coordinates)
        let country = await store.updateEventCountry(Event(
            eventType: viewModel.eventType,
            date: start,
            location: selectedLocation,
