...
class EventCountryGeocoder {
    ...
    static func previewCountryUpdate(for event: Event) async -> EventCountryPreview {
        ...
        let currentCountry = eventCountry ?? locationCountry
        
        print("🔍 [Preview] Event '\(event.city ?? "Unknown")': event.country='\(eventCountry ?? "nil")', location.country='\(locationCountry ?? "nil")', current='\(currentCountry ?? "nil")'")
        
        // Already has valid country
        if let country = currentCountry, !country.isEmpty && country.lowercased() != "unknown" {
            print("✅ [Preview] Event '\(event.city ?? "Unknown")': Already has valid country: \(country)")
        ...
        print("🔍 [Preview] Final result for '\(event.city ?? "Unknown")': proposed='\(proposedCountry ?? "nil")', source=\(source)")
    ...
    static func generatePreview(store: DataStore, progressCallback: @escaping (CountryUpdateProgress) -> Void) async -> [EventCountryPreview] {
        // First, filter to only "Other" location events
        let otherLocationEvents = store.events.filter { $0.location.name == "Other" }
        print("📋 [Preview] Total 'Other' location events: \(otherLocationEvents.count)")
        
        // Then filter to those needing country updates
        let eventsNeedingCountry = otherLocationEvents.filter { event in
            let eventCountry = event.country
            let locationCountry = event.location.country
            let combinedCountry = eventCountry ?? locationCountry
            
            // Needs update if: country is nil, empty string, or "Unknown"
            guard let country = combinedCountry else { return true }
            return country.isEmpty || country.lowercased() == "unknown"
        }
        
        print("📋 [Preview] Analyzing \(eventsNeedingCountry.count) events needing country updates...")
    ...
    static func updateCountry(for event: Event, store: DataStore) async -> Event? {
        ...
        print("🔄 [Update] Before - event.country='\(eventCountry ?? "nil")', location.country='\(locationCountry ?? "nil")', current='\(currentCountry ?? "nil")'")
        
        // Already has valid country from location or event
        if let country = currentCountry, !country.isEmpty && country.lowercased() != "unknown" {
            print("⏭️ [Update] Event already has valid country: \(country)")
        ...
        print("✅ [Update] After - Created updated event with country: \(country)")
        return updatedEvent
    }
